...
int main(int argc, char *argv[]) {
    ...
    const char *prog_name = CHILD_PARAMS[0].c_str();
    for (const auto &fname : DataActionImpl::getFiles()) {
        const char **child_argv = (const char **) NULL;
        unsigned int child_argc = 3 + CHILD_PARAMS.size();
        child_argv = new const char *[child_argc];
        
        child_argv[0] = prog_name;
        for (unsigned int p = 1U; p < CHILD_PARAMS.size(); p++) {
            child_argv[p] = CHILD_PARAMS[p].c_str();
        }
        child_argv[child_argc - 3] = fname.c_str();
        child_argv[child_argc - 2] = "y";
        child_argv[child_argc - 1] = (const char *) NULL;
